...
class PixivNovelPlugin implements Plugin.PagePlugin {
  ...
  private pixivContentToHtml(content: string): string {
    ...
    html = html.replace(/\[chapter:([^\]]+)\]/g, '<h2>$1</h2>');

    // [newpage] -> page separator
    html = html.replace(/\[newpage\]/g, '<hr style="margin: 2em 0;"/>');

    // [pixivimage:id] or [pixivimage:id-p#] -> embedded illustration
    html = html.replace(
      /\[pixivimage:(\d+)(?:-p(\d+))?\]/g,
      '<div style="text-align:center;margin:1em 0;"><img src="https://embed.pixiv.net/decorate.php?illust_id=$1" style="max-width:100%;" /></div>',
    );

    // [uploadedimage:id] -> uploaded image within novel
    html = html.replace(
      /\[uploadedimage:(\d+)\]/g,
      '<div style="text-align:center;margin:1em 0;">[Image]</div>',
    );

    // Ruby text: [[rb: text > ruby]]
    html = html.replace(
      /\[\[rb:\s*([^\s>]+)\s*>\s*([^\]]+)\]\]/g,
      '<ruby>$1<rp>(</rp><rt>$2</rt><rp>)</rp></ruby>',
    );

    // [jump:pageNumber] -> link within pages
    html = html.replace(/\[jump:(\d+)\]/g, '<a href="#page$1">[→ Page $1]</a>');
